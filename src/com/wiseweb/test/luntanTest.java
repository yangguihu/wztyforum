...
//		long start = 0L;
		...
		// data_content_31 --> 31
//		String keyid = table.substring(13);
		String keyid = "35";
...
//				}
				...
				String sql = "SELECT * FROM  data_content_33 WHERE Id = 6841";
				selpst = conn.prepareStatement(sql);
//				selpst.setInt(1, 5201);
//				selpst.setInt(2, pagenum);
...
//				} else { // 第一次取数据就为空
//					try {
//						JdbcUtil2.close(conn, selpst, rs);
				...
				while (rs.next()) {
					for (Entry<String, String> entry : RecordMaping.resMap
						...
						if ("site_name".equals(key)) {
							...
							value = DateUtil.FormatDate(value.toString());
						}
...
//			} else {
//				// 数据没有跑完不关 conn
//				JdbcUtil2.close(null, selpst, rs);
//			}
			JdbcUtil2.close(conn, selpst, rs);
//		}
		// 关闭zookeeper
